    ...
    click_confirm: function(){
        ...
        if (count > 0){
            ...
            for (var r=0, n = myTable.rows.length; r < n; r++) {
                ...
                if (return_qty > 0){
                    this.pos.get_order().add_new_product(product, {
...
//        this.$('.searchbox input').on('Enter',function(event){
//            clearTimeout(search_timeout);
//
//            var searchbox = this;
//
...
//        });
    ...
    render_list: function(orders){
        ...
        for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
            if (orders[i]['session_id'][0] == this.pos.pos_session.id)
            {
                var order = orders[i];
...
var ReturnButton = screens.ActionButtonWidget.extend({
    ...
    button_click: function(){
        var self = this;
        self.getSession().user_has_group('product_return_pos.group_retrun_button').then(function(has_group) {
        if(has_group) {
        var orders = self.pos.orders;
	    var fields = ['name', 'partner_id','date_order','amount_total', 'amount_tax','pos_reference','lines','state','session_id','company_id','return_ref','return_status','line_id'];
        var pos_order_domain = [['session_id', '=',self.pos.pos_session.id]];
        var load_orders = [];
        rpc.query({
            model: 'pos.order',
            method: 'search_read',
            ...
            load_orders = output;
            self.pos.orders = load_orders;
            orders = self.pos.orders;
            self.gui.show_screen('orderlist',{orders:orders});
        });
        }
        else{
...
var SubmitOrderButton1 = screens.ActionButtonWidget.extend({
    ...
    button_click: async function(){
        var self = this;
        self.getSession().user_has_group('custom_pos_receipt.group_kitchen_print_button').then(function(has_group) {
            if(has_group){
                self.print_kitchen();
            }
            else{
...
models.Order = models.Order.extend({
    ...
    add_new_product: function (product, options) {
