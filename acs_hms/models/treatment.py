...
class ACSTreatment(models.Model):
    ...
    department_id = fields.Many2one('hr.department', ondelete='restrict', string='Department',
        domain=[('patient_depatment', '=', True)], states=READONLY_STATES, tracking=True)
    ...
    def create_invoice(self):
        ...
        invoice = self.acs_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=[{'product_id': product_id}])
