...
class PatientLabTestLine(models.Model):
    ...
    test_type = fields.Selection([
        ('pathology','Pathology'),
        ('radiology','Radiology'),
    ], related='test_id.test_type', string='Test Type', readonly=True)
...
class LaboratoryRequest(models.Model):
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pricelist_context).acs_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data={})
    ...
    def action_view_lab_samples(self):
        action = self.env.ref('acs_hms_laboratory.action_acs_patient_laboratory_sample').read()[0]
        action['domain'] = [('request_id','=',self.id)]
    ...
    def action_sendmail(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('acs_hms_laboratory', 'acs_lab_req_email')[1]
        except ValueError:
            template_id = False
        try:
