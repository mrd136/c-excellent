...
class ReportPosConsumption(models.AbstractModel):

    _name = 'report.pos_mrp_order.consumption_report'
    
    _description = 'Point of Consumption'

    @api.model
    def get_consumption_details(self, date_start=False, date_stop=False, config_ids=False, sessions=False):
    ...
    def get_payment(self,session_id=False):
        ...
        return total
    def get_order_line(self,session_id=False):
        session = self.env['pos.session'].search([('id','=',session_id.id)])
        orders = self.env['pos.order'].search([('session_id', '=', session.id)])
        total = 0
        for r in orders:
    ...
    def _get_report_values(self, docids, data=None):
        ...
        print("SSSSSSSSSSSSSSSSSSSSSS",sessions)
       
         
        configs = data['config_id']
        data.update(self.get_consumption_details(data['date_start'], data['date_stop'], configs, sessions))
        return data
