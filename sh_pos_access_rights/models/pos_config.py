    ...
    def _compute_access_rights(self):
        for rec in self:
            rec.disable_payment_id = self.env.ref(
                'sh_pos_access_rights.group_disable_payment')
            rec.group_select_customer = self.env.ref(
                'sh_pos_access_rights.group_select_customer')
            rec.group_disable_discount = self.env.ref(
                'sh_pos_access_rights.group_disable_discount')
            rec.group_disable_qty = self.env.ref(
                'sh_pos_access_rights.group_disable_qty')
            rec.group_disable_price = self.env.ref(
                'sh_pos_access_rights.group_disable_price')
            rec.group_disable_plus_minus = self.env.ref(
                'sh_pos_access_rights.group_disable_plus_minus')
            rec.group_disable_numpad = self.env.ref(
                'sh_pos_access_rights.group_disable_numpad')
