...
class HMSWebsite(Website):
    ...
    def create_booking_data(self):
        ...
        department_ids = request.env['hr.department'].sudo().search([('patient_depatment','=',True),('allowed_online_booking','=',True)])
    ...
    def user_booking_data(self, post):
        ...
        appoitment_by = post.get('appoitment_by','')
    ...
    @http.route(['/get/appointment/data'], type='http', auth='public', website=True)
    ...
    @http.route(['/get/appointment/personaldata'], type='http', auth='public', website=True)
