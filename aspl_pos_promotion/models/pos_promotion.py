...
class pos_config(models.Model):
    ...
    applied_promotion = fields.Selection([('manual','Manual'),
                                          ('automatic','Automatic')], string='Apply Promotion', default='manual', required=True)
...
class Pos_promotion(models.Model):
    ...
    promotion_type = fields.Selection([('buy_x_get_y','Buy X Get Y Free'),
                                       ...
                                       ('buy_x_categ_get_dis_y_categ', 'Buy X Category Get Discount On Y Category'),
...
#     ('discount_on_above_price','Discount On Above Price')
    ...
    get_x_categ_disc_y_categ = fields.One2many('buy.x.categ.disc.y.categ','multi_x_categ_disc_y_rel')
    ...
    pos_ids = fields.Many2many(comodel_name='pos.config', string="Points Of Sale")
...
class quantity_discount_amt(models.Model):
    ...
    discount_price = fields.Float('Product Price (Fixed)')
...
class buy_x_categ_disc_y_categ(models.Model):
    _name = 'buy.x.categ.disc.y.categ'

    multi_x_categ_disc_y_rel = fields.Many2one('pos.promotion')
    x_categ_ids = fields.Many2many('pos.category', 'buy_rel_1', 'buy_id', 'categ_id', string="Categories X")
    operator = fields.Selection([('is_eql_to', 'Is Equal To'),
                                 ('greater_than_or_eql', 'Greater Than Or Equal')])
    x_categ_qty = fields.Float("Product X Qty")
    y_categ_ids = fields.Many2many('pos.category','buy_rel_2', 'buy_id', 'categ_id', string="Categories Y")
