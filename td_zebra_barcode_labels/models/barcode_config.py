...
class TDZebraBarcodeConfiguration(models.Model):
    ...
    def _get_barcode_field(self):
        field_list = []
        ir_model_id = self.env['ir.model'].search(['|', ('model', '=', 'product.product'),('model', '=', 'stock.production.lot')])
        for model in ir_model_id:
            if model:
    ...
    barcode_field_sn = fields.Selection('_get_barcode_field', string="S/N Barcode Field")
    ...
    product_name_upr_bar_size = fields.Char('ProductName(Uper Barcode)FontSize', default=10)
