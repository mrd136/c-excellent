...
class TRReportBarcodeLabels(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for rec in data['form']['product_ids']:
            for loop in range(0, int(rec['qty']) * 2):
        ...
        return {
            ...
            'get_sn_barcode_string': self._get_sn_barcode_string,
            'data': data,
    ...
    @api.model
    def _get_sn_barcode_string(self, sn_barcode_value, data):
        barcode_str = barcode.createBarcodeDrawing(
                            data['barcode_type'],
                            value=sn_barcode_value,
                            format='png',
                            width=int(data['barcode_height']) + 2,
                            height=int(data['barcode_width']) + 20,
                            humanReadable=data['humanreadable']
                            )
        encoded_string = b64encode(barcode_str.asString('png')).decode("utf-8")
        barcode_str = "<img style='width:" + str(data['display_width']+ 20) + "px;height:" + str(data['display_height'] + 2) + "px'src='data:image/png;base64,{0}'>".format(encoded_string)
    ...
    def _get_lines(self, form):
        ...
        for l in lines:
            ...
            lot = lot_obj.sudo().search([('name', '=', l['barcode_field_sn'])],limit=1)
            for c in range(0, int(l['qty']) * 2):
                value = total_value % 2
                data_append = False
                ...
                sn_barcode_value = lot.name #[str(form['barcode_field_sn'])]
                ...
                dict_data.update({
                   ...
                   'ncode'+'_'+str(value): sn_barcode_value,
