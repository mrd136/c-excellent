                ...
                for line in picking.move_lines:
                    if line.product_id and line.product_id.type != 'service':
                        product_get_ids += [(0, 0, {
                                 'product_id': line.product_id.id,
                                 'qty': int(abs(line.product_qty)) or 1.0,
                                 'lot_id': self.env['stock.move.line'].search([('move_id', '=', line.id),
                                                                               ('product_id', '=', line.product_id.id)], limit=1).lot_id.id,
                                 })]
    ...
    def print_report(self):
        ...
        datas = {
                 ...
                 'form': {
                    ...
                    # 'barcode_field_sn': config_rec.barcode_field_sn or '',
                    ...
                    'product_ids': [{
                         'product_id': line.product_id.id,
                         'qty': line.qty,
                         'barcode_field_sn': line.lot_id.name,
                         } for line in self.product_get_ids]
                      }
                 }
        for x in self.product_get_ids: 
            product = self.env['product.product'].browse([x.product_id.id])
            lot = self.env['stock.production.lot'].browse([x.lot_id.id])
            barcode_value = product[config_rec.barcode_field]
            barcode_value_sn = lot.name #product[config_rec.barcode_field_sn]
