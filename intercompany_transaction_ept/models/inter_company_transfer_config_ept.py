...
class InterCompanyTransferConfigEpt(models.Model):
    ...
    bills_control = fields.Selection([('purchase', 'Based on Ordered Quantity'),
                                      ('receive', 'Based on Received Quantity')],
                                     default='purchase', string="Bills Control")
    company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id,
                                 readonly=True)

    @api.onchange('bills_control')
    def onchange_bills_control(self):
        if self.bills_control == 'purchase':
            self.env.cr.execute("""
                                    UPDATE product_template
                                    SET purchase_method = 'purchase'
                                """)
        else:
            self.env.cr.execute("""UPDATE product_template
